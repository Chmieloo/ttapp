    ...
    <div class="mainContainer">
      ...
      <table class="table-player-list">
        <tr class="row-header">
          ...
          <th class="txt-right" style="width: 100px">
        ...
        >
          <td>{{ index + 1 }}</td>
          <td class="txt-left player-link">
            <router-link :to="'/player/' + player.id + '/info'">{{
              player.name
            }}</router-link>
          </td>
          <td class="txt-right">
            <span v-if="player.gamesPlayed >= 15" style="color: white">{{
              player.elo
            }}</span>
            <span v-else>?</span>
          </td>
          <td class="txt-right">
            ...
            <span style="margin-left: 10px">
              <span
                style="color: red"
                v-if="player.elo - player.oldElo <= -100"
              >
                <i class="fas fa-angle-double-down"></i>
              </span>
              <span
                style="color: red"
                v-else-if="player.elo - player.oldElo < 0"
              >
                <i class="fas fa-angle-down"></i>
              </span>
              <span
                style="color: green"
                v-else-if="player.elo - player.oldElo > 100"
              >
  ...
  computed: {
    filteredPlayers: function () {
      return this.players.filter((player) => {
        return parseInt(player.officeId) === parseInt(this.officeId);
      });
    },
  },
  methods: {
