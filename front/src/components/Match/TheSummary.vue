<template>
  <div class="mainContainer" v-if="this.matchData">
    <div>
      <span class="header-title"> MATCH SUMMARY </span>
    </div>
    <div>
    ...
    <div class="setContainer">
      <table style="border-collapse: collapse; color: white">
        <tr style="height: 40px; line-height: 40px">
          <td></td>
          <td style="min-width: 100px; color: white; font-weight: 600">
            SCORE
          </td>
          <td
            style="min-width: 100px; color: #8e8e8e; font-weight: 600"
            v-for="(setData, index) in setsData"
            v-bind:key="index"
          >
            SET {{ index }}
          </td>
        ...
        <tr style="height: 40px; line-height: 40px">
          <td class="playerName">
            <span
              v-if="
                parseInt(matchData.homeTotalScore) >
                parseInt(matchData.awayTotalScore)
              "
              class="winnerColor"
            >
              {{ homeName }}
          ...
          <td>
            {{ matchData.homeTotalPoints }}
            <span style="color: #8e8e8e"
              >( {{ matchData.homePointsPerc }}% )</span
            >
          </td>
          <td>
            {{ matchData.homeOwnServePointsTotal }} /
            {{ matchData.homeServesTotal }}
            <span style="color: #8e8e8e"
              >( {{ matchData.homeServePointsPerc }}% )</span
            >
          </td>
        </tr>
        <tr style="height: 40px; line-height: 40px">
          ...
          <td style="min-width: 100px">
            {{ matchData.awayTotalPoints }}
            <span style="color: #8e8e8e"
              >( {{ matchData.awayPointsPerc }}% )</span
            >
          </td>
          <td>
            {{ matchData.awayOwnServePointsTotal }} /
            {{ matchData.awayServesTotal }}
            <span style="color: #8e8e8e"
              >( {{ matchData.awayServePointsPerc }}% )</span
            >
          </td>
        </tr>
      </table>
    ...
    <div>
      <span class="header-title"> SETS SUMMARY </span>
    </div>
      ...
      <div class="setContainer">
        <table style="border-collapse: collapse">
          <tr>
            ...
            >
              <span
                v-if="pointData.currentServer == pointData.homePlayerId"
                class="serve"
              >
          ...
          <tr style="height: 40px; line-height: 40px">
            <td class="playerName">
              <span
                v-if="
                  parseInt(setData.setSummary.homePoints) >
                  parseInt(setData.setSummary.awayPoints)
                "
                class="winnerColor"
              >
                {{ homeName }}
          ...
          <tr>
            ...
            >
              <span
                v-if="pointData.currentServer == pointData.awayPlayerId"
                class="serve"
              >
                <i class="fas fa-table-tennis"></i>
              </span>
          ...
          <table>
            ...
            <tr>
              ...
              <td
                ...
                "
              >
                {{ setData.setSummary.durationMinutes }}:{{
                  setData.setSummary.durationSeconds
                }}
              </td>
              ...
              >
                <div>
                  {{ setData.setSummary.homeServePoints }}
                  <span style="color: #9a9a9a">/</span>
                  {{ setData.setSummary.homeServes }}
                  <span style="color: #9a9a9a">/</span>
                  {{ setData.setSummary.homeServePointsPerc }}%
                </div>
                <div>
                  {{ setData.setSummary.awayServePoints }}
                  <span style="color: #9a9a9a">/</span>
                  {{ setData.setSummary.awayServes }}
                  <span style="color: #9a9a9a">/</span>
    ...
    <div>
      <span class="header-title"> SPECTATORS </span>
    </div>
    <div class="setContainer">
...
export default {
  ...
  mounted() {
    this.idle = false;
    axios
      .get("/api/matches/" + this.$route.params.id + "/timeline")
      .then((res) => {
    ...
    axios
      .get("/api/matches/" + this.$route.params.id + "/spectatortimeline")
      .then((res) => {
