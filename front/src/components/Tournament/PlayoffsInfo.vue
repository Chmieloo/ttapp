  ...
  <div class="mainContainer">
    ...
    <table class="tbl" v-for="match in this.data" v-bind:key="match.id">
      <tr class="playoffsRow">
        ...
        <td
          v-bind:class="
            match.winnerId != 0 && match.winnerId == match.homePlayerId
              ? 'module winner-class'
              : 'module normal-class'
          "
        ...
        <td class="module" style="background-color: #105869; width: 30px">
          &nbsp;
        </td>
        <td
          style="
        ...
        <td
          v-bind:class="
            match.winnerId != 0 && match.winnerId == match.awayPlayerId
              ? 'module winner-class'
              : 'module normal-class'
          "
        ...
        <td style="padding-left: 30px">
          {{ match.division }}, {{ match.name }}
        </td>
        <td style="text-align: left; width: 120px">
          <span v-if="match.status == 1"> finished </span>
          <span v-else-if="match.status == 2" class="blinking"> live </span>
          <span v-else> not started </span>
        </td>
        <td style="width: 100px; text-align: left">
          <span v-if="match.isFinished == 0">
            <router-link :to="'/match/' + match.matchId + '/spectate'"
              >watch</router-link
            >
          </span>
          <span v-else-if="match.isWalkover == 1"> walkover </span>
          <span v-else>
            <router-link :to="'/match/' + match.matchId + '/summary'"
              >summary</router-link
            >
          </span>
        </td>
      ...
      <tr
        ...
        style="background-color: #022d38; border: 1px solid #0e3f4a"
      >
        <td>&nbsp;</td>
